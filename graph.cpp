...
void Graph::createInstance(int n) {
    ...
    // Inicializar matriz de adjac̻ncias
    vector<double> v(n, 0);
    ...
    for (int i = 0; i < this->numberVertices; i++) {
        ...
        // Gerar coordenadas dos v̩rtices em [0,100]
        newVertex.x = coordinate(generator);
        newVertex.y = coordinate(generator);

        if (i > 0) {

            // Gerar probabilidade de presen̤a e demanda dos clientes
...
void Graph::computeDistances() {
    ...
    for (int i = 0; i < this->numberVertices; i++) {
        for (int j = 0; j < i; j++) {
            ...
            // Salvar a dist̢ncia euclidiana entre os v̩rtices
